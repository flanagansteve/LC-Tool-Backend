...
# TODO lc.client_approved might be redundant and in fact inconvenient if the client expects the issuer to handle
#  negotiations
...
class LC(models.Model):
    ...
    beneficiary = models.ForeignKey(Business, on_delete=models.CASCADE,
                                    related_name='%(app_label)s_%(class)s_beneficiary', null=True, blank=True)
    issuer = models.ForeignKey(Bank, on_delete=models.CASCADE, related_name='%(app_label)s_%(class)s_issuer')
    account_party = models.ForeignKey(Business, on_delete=models.CASCADE,
    ...
    tasked_beneficiary_employees = models.ManyToManyField(BusinessEmployee,
                                                          related_name='%(app_label)s_%('
                                                                       'class)s_tasked_beneficiary_employees')
    tasked_issuer_employees = models.ManyToManyField(BankEmployee,
                                                     related_name='%(app_label)s_%(class)s_tasked_issuer_employees')
    tasked_account_party_employees = models.ManyToManyField(BusinessEmployee,
                                                            related_name='%(app_label)s_%('
                                                                         'class)s_tasked_account_party_employees')
    tasked_advising_bank_employees = models.ManyToManyField(BankEmployee,
                                                            related_name='%(app_label)s_%('
                                                                         'class)s_tasked_advising_bank_employees')
    ...
    def get_base_fields(self):
        def boycott_to_dict():
            boycott_dict = {}
            for boycott in self.boycottlanguage_set.values():
                cur_list = boycott_dict[boycott['source']] if boycott['source'] in boycott_dict else []
                cur_list.append(boycott)
                boycott_dict[boycott['source']] = cur_list
            return boycott_dict
...
def pdf_app_response_path(lc, filename):
    ...
    return 'bank_{0}/client_{1}/applications/%Y/%m/%d/{2}'.format(instance.lc.issuer.id, instance.lc.client.id,
                                                                  filename)
...
class DigitalLC(LC):
    ...
    paying_other_banks_fees = models.ForeignKey(Business, on_delete=models.CASCADE,
                                                related_name='%(app_label)s_%(class)s_paying_other_banks_fees',
                                                null=True, blank=True)
    # almost always the issuer or the confirming bank
    credit_expiry_location = models.ForeignKey(Bank, on_delete=models.CASCADE,
                                               related_name='%(app_label)s_%(class)s_credit_expiry_location', null=True,
                                               blank=True)
    ...
    paying_acceptance_and_discount_charges = models.ForeignKey(Business, on_delete=models.CASCADE,
                                                               related_name='%(app_label)s_%('
                                                                            'class)s_paying_acceptance_and_discount_charges',
                                                               null=True, blank=True)
...
class BoycottLanguage(models.Model):
    lc = models.ForeignKey(LC, on_delete=models.CASCADE)
...
class CommercialInvoiceRequirement(DocumentaryRequirement):
    ...
    # These are product params - some purchases some day will have more than 1 product per CI, but for now we assume
    # theres 1 per
